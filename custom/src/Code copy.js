...
// tambahan qwen
   ...
   // === AUTH ROUTING (BARU) ===
    ...
    // === QUIZ / ATTENDANCE ROUTING (EXISTING) ===
    // Verifikasi user jika ada studentId
    if (data.studentId) {
      const user = verifyUser(data.studentId);
      if (user.status === "success") {
        data.name = user.nama; // Override nama dengan nama asli dari DB
      }
    }
...
function saveQuiz(data) {
  ...
  if (!sheet) {
    ...
    setupHeader(sheet, ["Timestamp", "Nama", "Skor (%)", "Total Soal", "Status"]);
  }
...
function saveAttendance(data) {
  ...
  if (!sheet) {
    sheet = ss.insertSheet(sheetName);
    setupHeader(sheet, ["Timestamp", "Tanggal", "Hari", "Nama", "Tipe Aktivitas", "Deskripsi", "Count"]);
  }
  ...
  sheet.appendRow([formattedTime, formattedDate, dayName, data.name, data.activityType || "activity", data.description || "Aktivitas", 1]);
...
function updateSummary() {
  ...
  allSheets.forEach(sheet => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (!studentStats[studentName]) {
        studentStats[studentName] = { totalKuis: 0, totalScore: 0, highestScore: 0, lowestScore: 100, reading: 0, quizActivity: 0, discussion: 0, download: 0, totalActivity: 0, pertemuan: [], lastQuizStatus: "" };
      }
  ...
  const headers = ["Nama", "Total Kuis", "Rata-rata Skor", "Skor Tertinggi", "Skor Terendah", "Total Aktivitas", "Reading", "Quiz Activity", "Discussion", "Download", "Jumlah Pertemuan", "Status Kuis Terakhir"];
  ...
  Object.entries(studentStats).forEach(([studentName, stats]) => {
    ...
    summarySheet.getRange(rowNum, 1, 1, headers.length).setValues([[
      studentName, stats.totalKuis, avg, stats.highestScore, stats.lowestScore,
...
// ============================================
// AUTH SYSTEM — Tambahkan di bawah kode v3
// ============================================

/**
 * Sheet "Users" structure:
 * | StudentID | Nama | Email | RegisteredAt | LastLogin |
 */
...
function registerUser(data) {
  ...
  if (!sheet) {
    ...
    setupHeader(sheet, ["StudentID", "Nama", "Email", "RegisteredAt", "LastLogin"]);
  }
  ...
  for (let i = 1; i < existing.length; i++) {
    if (String(existing[i][2]).trim().toLowerCase() === email) {
      return { 
        ...
        studentId: existing[i][0],
        nama: existing[i][1]
      };
    }
  }

  // Generate Student ID unik
  ...
  sheet.appendRow([studentId, nama, email, now, now]);
  sheet.autoResizeColumns(1, 5);
...
function loginUser(data) {
  ...
  return { status: "error", message: "Email tidak ditemukan. Silakan register dulu." };
}
...
function verifyUser(studentId) {
  ...
  if (!sheet || sheet.getLastRow() <= 1) return { status: "error", message: "User tidak ditemukan" };
