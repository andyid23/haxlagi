...
class FullQuizDashboard extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // FIX: ketika sudah connected, langsung pindah ke tab quiz (jangan stuck di setup)
    if (this._connected && this._tab === 'setup') this._tab = 'quiz';
  ...
  _connect(url) {
    ...
    this._show('URL sudah di-hardcode di HTML. Edit atribut apps-script-url untuk mengganti.');
    this._render();
  }
    ...
    return `
      <div class="c">
        <div class="sc">
          ...
          <div class="ti">💡 Tip: Anda juga bisa set atribut <code>apps-script-url</code> langsung di tag HTML.</div>
        </div>
      </div>
    `;
  }
