...
class ExplodeQuiz extends I18NMixin(DDDSuper(LitElement)) {
  ...
  _renderShortAnswer(q) {
    return html`<div class="short-answer-container"> <input type="text" class="short-answer-input" ?disabled="${this._answered}" placeholder="Ketik jawaban..." .value="${this._shortAnswerText}" @input="${(e) => { this._shortAnswerText = e.target.value }}" /> ${!this._answered ? html`<button class="start-btn multi-submit" @click="${() => this._submitShortAnswer()}" > Kirim Jawaban </button>`: ""} </div>`
  }
  ...
  _renderEditorScreen() {
    ...
    return html`<header class="edit-header"> <h2 class="edit-title">${this.t.editTitle}</h2> <button class="close-editor-btn" @click="${this._saveAll}"> ${this.t.closeEditor} </button> </header> <div class="editor-content"> <form class="add-question-form"> <div class="editor-toolbar"> <select class="editor-select" .value="${qType}" @change="${(e) => { this._tempQuestionType = e.target.value }}" > <option value="mc">Pilihan Ganda</option> <option value="pgk">PG Kompleks (Benar/Salah)</option> <option value="matching">Menjodohkan</option> <option value="shortAnswer">Isian Singkat</option> </select> <input type="text" class="editor-input" placeholder="URL gambar (opsional)" .value="${this._tempQuestionImage}" @input="${(e) => { this._tempQuestionImage = e.target.value }}" /> <label class="editor-label">Poin:</label> <input type="number" min="1" class="editor-input-small" .value="${String(this._tempQuestionPoints)}" @input="${(e) => { this._tempQuestionPoints = parseInt(e.target.value) || 1 }}" /> </div> ${this._tempQuestionImage ? html` <div class="editor-image-preview"> <img src="${this._tempQuestionImage}" /> </div>`: ""} <textarea class="question-text-input" .value="${this._tempQuestionText}" @input="${(e) => (this._tempQuestionText = e.target.value)}" placeholder="${this.t.questionPlaceholder}" ></textarea> ${qType === "mc" ? this._renderEditorMC() : ""} ${qType === "pgk" ? this._renderEditorPGK() : ""} ${qType === "matching" ? this._renderEditorMatching() : ""} ${qType === "shortAnswer" ? this._renderEditorShortAnswer() : ""} <button type="button" class="add-question-btn" @click="${this._addQuestion}" > ${this.t.addQuestionBtn} </button> </form> <div class="questions-list"> ${this._tempQuestions.map( (question, index) => html` <div class="question-card"> ${this._editingIndex === index ? html`<div class="edit-form"> <div class="editor-toolbar"> <select class="editor-select" .value="${this._tempQuestionType}" @change="${(e) => { this._tempQuestionType = e.target.value }}" > <option value="mc">Pilihan Ganda</option> <option value="pgk">PG Kompleks</option> <option value="matching">Menjodohkan</option> <option value="shortAnswer">Isian Singkat</option> </select> <input type="text" class="editor-input" placeholder="URL gambar" .value="${this._tempQuestionImage}" @input="${(e) => { this._tempQuestionImage = e.target.value }}" /> <label class="editor-label">Poin:</label> <input type="number" min="1" class="editor-input-small" .value="${String(this._tempQuestionPoints)}" @input="${(e) => { this._tempQuestionPoints = parseInt(e.target.value) || 1 }}" /> </div> <textarea class="edit-question-text-input" .value="${this._tempQuestionText}" @input="${(e) => (this._tempQuestionText = e.target.value)}" placeholder="${this.t.questionPlaceholder}" ></textarea> ${this._tempQuestionType === "mc" ? this._renderEditorMC() : ""} ${this._tempQuestionType === "pgk" ? this._renderEditorPGK() : ""} ${this._tempQuestionType === "matching" ? this._renderEditorMatching() : ""} ${this._tempQuestionType === "shortAnswer" ? this._renderEditorShortAnswer() : ""} <div class="edit-form-actions"> <button type="button" class="save-edit-btn" @click="${this._saveEditQuestion}" > ${this.t.saveEditBtn} </button> <button type="button" class="cancel-edit-btn" @click="${this._cancelEditQuestion}" > ${this.t.cancelEditBtn} </button> </div> </div>` : html`<div class="question-card-header"> <div> <strong class="question-type-badge" >[${(question.type || "mc").toUpperCase()}]</strong> ${question.question} ${question.image ? html` <span class="question-image-indicator" >[gambar]</span>`: ""} <span class="question-points-indicator" >[${question.points || 1} poin]</span> </div> <div class="question-card-actions"> <button class="edit-btn" @click="${() => this._startEditQuestion(index)}" > ${this.t.editQuestionBtn} </button> <button class="delete-btn" @click="${() => this._deleteQuestion(index)}" > ${this.t.deleteQuestionBtn} </button> </div> </div>`} </div> `, )} </div> </div>`
  }

  _renderEditorMC() {
    return html`<div class="choices-container"> ${[0, 1, 2, 3].map( (index) => html` <div class="choice-input-wrapper"> <input class="choice-input" .value="${this[`_tempChoice${index}`]}" @input="${(e) => (this[`_tempChoice${index}`] = e.target.value)}" placeholder="${this.t.choicePlaceholder.replace( "{N}", index + 1, )}" /> <input type="url" class="choice-image-input" placeholder="🖼️ URL gambar" .value="${this[`_tempChoiceImage${index}`] || ""}" @input="${(e) => (this[`_tempChoiceImage${index}`] = e.target.value)}" /> <label class="choice-label"> <input type="checkbox" ?checked="${this._tempCorrectAnswers.includes(index)}" @change="${(e) => { if (e.target.checked) { this._tempCorrectAnswers = [ ...this._tempCorrectAnswers, index, ] } else { this._tempCorrectAnswers = this._tempCorrectAnswers.filter( (i) => i !== index, ) } if (this._tempCorrectAnswers.length <= 1) { this._tempCorrectIndex = index.toString() } }}" /> ${this.t.choiceCorrectLabel} </label> ${this[`_tempChoiceImage${index}`] ? html`<img src="${this[`_tempChoiceImage${index}`]}" class="choice-image-preview" />` : ""} </div> `, )} ${this._tempCorrectAnswers.length <= 1 ? html` <div class="editor-hint"> Pilih 1 jawaban benar. Centang lebih dari 1 untuk mode PG Kompleks. </div>`: html` <div class="editor-hint editor-hint--accent"> Mode PG Kompleks: ${this._tempCorrectAnswers.length} jawaban benar dipilih </div>`} </div>`
  }

  _renderEditorPGK() {
    return html`<div class="editor-pgk-container"> <div class="editor-hint"> Pernyataan (JSON array, format: [{"text":"...","answer":true}]): </div> <textarea class="editor-json-textarea" .value="${this._tempAcceptedStatements}" @input="${(e) => { this._tempAcceptedStatements = e.target.value }}" ></textarea> </div>`
  }

  _renderEditorMatching() {
    return html`<div class="editor-matching-container"> <div class="editor-matching-columns"> <div class="editor-matching-column"> <div class="editor-hint">Item Kiri:</div> ${this._tempLeftItems.map( (item, i) => html` <input class="editor-input" .value="${item}" @input="${(e) => { const a = [...this._tempLeftItems]; a[i] = e.target.value; this._tempLeftItems = a; }}" placeholder="Item ${i + 1}" /> `, )} <button type="button" class="editor-add-btn" @click="${() => { this._tempLeftItems = [...this._tempLeftItems, ""] }}" > + Tambah </button> </div> <div class="editor-matching-column"> <div class="editor-hint">Item Kanan:</div> ${this._tempRightItems.map( (item, i) => html` <input class="editor-input" .value="${item}" @input="${(e) => { const a = [...this._tempRightItems]; a[i] = e.target.value; this._tempRightItems = a; }}" placeholder="Item ${String.fromCharCode(65 + i)}" /> `, )} <button type="button" class="editor-add-btn" @click="${() => { this._tempRightItems = [...this._tempRightItems, ""] }}" > + Tambah </button> </div> </div> <div class="editor-hint"> Kunci Jawaban (JSON: {"0":1,"1":0} artinya Item Kiri 0→Item Kanan B): </div> <input class="editor-json-input" .value="${JSON.stringify(this._tempCorrectPairs)}" @input="${(e) => { try { this._tempCorrectPairs = JSON.parse(e.target.value) } catch (_) {} }}" /> </div>`
  }

  _renderEditorShortAnswer() {
    return html`<div class="editor-short-answer-container"> <div class="editor-hint">Jawaban yang diterima (pisahkan koma):</div> <input class="editor-input" placeholder="contoh: biomassa, sekam padi, limbah pertanian" .value="${this._tempAcceptedAnswers}" @input="${(e) => { this._tempAcceptedAnswers = e.target.value }}" /> </div>`
  }
