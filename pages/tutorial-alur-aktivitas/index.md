...
`{Pertemuan} - Aktivitas` (type=reading)

`activity-logger._handleScroll`
...
`{Pertemuan} - Aktivitas` (type=assignment)

`assignment-saved` event
...
`{Pertemuan} - Aktivitas` (type=download)

`download-saved` event
...
`Forum Log` (via `forumApiUrl`)

`discussion-saved` event
